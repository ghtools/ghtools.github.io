...
(() => {
  ...
  /* ---------- QR mode: Link/Text, WiFi, or Business Card (vCard) ---------- */
  let qrMode = 'text';
  const modeFields = {
    text: document.getElementById('qr-mode-text'),
    wifi: document.getElementById('qr-mode-wifi'),
    vcard: document.getElementById('qr-mode-vcard'),
  };
  const typeRow = document.getElementById('qr-type-row');

  function escapeWifi(str) { return (str || '').replace(/([\\;,:"])/g, '\\$1'); }
  function escapeVCard(str) { return (str || '').replace(/([\\;,])/g, '\\$1'); }
  ...
  function buildPayload() {
    if (qrMode === 'wifi') {
      ...
      if (!ssid) return '';
      const passPart = enc === 'nopass' ? '' : `P:${escapeWifi(password)};`;
      return `WIFI:T:${enc};S:${escapeWifi(ssid)};${passPart}H:${hidden ? 'true' : 'false'};;`;
    }
    if (qrMode === 'vcard') {
  ...
  function switchMode(newMode) {
    qrMode = newMode;
    Object.entries(modeFields).forEach(([key, el]) => { if (el) el.hidden = key !== newMode; });
    typeRow?.querySelectorAll('.template-pick').forEach(p => {
      const isActive = p.dataset.type === newMode;
  ...
  typeRow?.querySelectorAll('.template-pick').forEach(pick => {
    pick.addEventListener('click', () => switchMode(pick.dataset.type));
    pick.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); switchMode(pick.dataset.type); }
    });
  });

  ['wifi-ssid', 'wifi-password', 'wifi-encryption', 'wifi-hidden', 'vcard-name', 'vcard-phone', 'vcard-email', 'vcard-company'].forEach(id => {
  ...
  async function share() {
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        ...
        await navigator.share({ title: 'QR Code', text: buildPayload() });
        toast('Shared.', 'success');
  ...
  function clearAll() {
    if (qrMode === 'wifi') {
      ...
      ['vcard-name', 'vcard-phone', 'vcard-email', 'vcard-company'].forEach(id => { document.getElementById(id).value = ''; });
    } else {
      els.text.value = '';
    }
