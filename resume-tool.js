...
(() => {
  ...
  document.querySelectorAll('.template-pick').forEach(pick => {
    pick.addEventListener('click', () => {
      ...
      doc.classList.remove('template-modern', 'template-classic', 'template-minimal', 'template-creative');
  ...
  function saveData() {
    try {
      const data = {
        ...
        template: ['modern', 'classic', 'minimal', 'creative'].find(t => doc.classList.contains('template-' + t)) || 'modern',
  ...
  function restoreData() {
    ...
    if (!data) {
      expBlock('Senior Software Engineer', 'TechCorp', '2022 — Present', 'Led development of the core platform API, improving performance by 40%.');
      eduBlock('Stanford University', 'B.Sc. Computer Science', '2016 — 2020');
      renderPreview();
      return;
    }
    ...
    (data.exp && data.exp.length ? data.exp : [{role:'Senior Software Engineer',company:'TechCorp',dates:'2022 — Present',desc:'Led development of the core platform API, improving performance by 40%.'}])
      .forEach(x => expBlock(x.role, x.company, x.dates, x.desc));
    (data.edu && data.edu.length ? data.edu : [{school:'Stanford University',degree:'B.Sc. Computer Science',dates:'2016 — 2020'}])
  ...
  async function downloadPdf(btn) {
    ...
    // Fix 1: html2canvas cannot reliably render through backdrop-filter
    // (glass blur) — temporarily switch it off site-wide during capture.
    document.body.classList.add('pdf-capturing');
    // Fix 2: neutralize the zoom transform/transition so we capture at
    // true 100% scale, not mid-animation or scaled down.
    zoomInner.style.transition = 'none';
    zoomInner.style.transform = 'none';
    void zoomInner.offsetHeight; // force reflow

    // Let the browser paint the style changes before capture.
    await new Promise(r => requestAnimationFrame(() => requestAnimationFrame(r)));

    try {
      // Best-effort pre-check: if html2canvas is separately exposed by the
      // bundle, sample the capture for real content before committing to a
      // save — catches "blank PDF" early. If html2canvas isn't available as
      // a standalone global in this bundle build, skip the check rather
      // than blocking the download (html2pdf's own internal call still runs).
      if (typeof html2canvas === 'function') {
        const testCanvas = await html2canvas(doc, { scale: 1, useCORS: true, backgroundColor: '#ffffff' });
        const ctx = testCanvas.getContext('2d');
        let hasContent = false;
        const cols = 12, rows = 16;
        for (let r = 0; r < rows && !hasContent; r++) {
          for (let c = 0; c < cols && !hasContent; c++) {
            const x = Math.floor((c + 0.5) * testCanvas.width / cols);
            const y = Math.floor((r + 0.5) * testCanvas.height / rows);
            const [red, green, blue] = ctx.getImageData(x, y, 1, 1).data;
            if (red < 245 || green < 245 || blue < 245) hasContent = true;
          }
        }
        if (!hasContent) throw new Error('blank-capture');
      }

      // Generate + save using html2pdf's own well-tested internal pipeline
      // (avoids relying on a window.jspdf global that isn't always exposed).
      const opts = {
        margin: [10, 10, 10, 10],
        filename: filename,
      ...
      if (e && e.message === 'blank-capture') {
        toast("The PDF came out blank — this can happen on some browsers. Try again, or use full-page zoom at 100%.", 'error');
      } else if (e && (e.name === 'SecurityError' || /cssRules|cross-?origin/i.test(e.message || ''))) {
        toast('A font file blocked the PDF export. Reload the page fully (fonts need to finish loading) and try again.', 'error');
      } else {
