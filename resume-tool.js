...
(() => {
  ...
  async function downloadPdf(btn) {
    ...
    try {
      ...
      // Verify the capture actually has content (sample a grid of points).
      // If html2canvas silently produced a blank/white canvas, we want to
      // know immediately rather than saving an empty PDF.
      const ctx = canvas.getContext('2d');
      let hasContent = false;
      const cols = 12, rows = 16;
      for (let r = 0; r < rows && !hasContent; r++) {
        for (let c = 0; c < cols && !hasContent; c++) {
          const x = Math.floor((c + 0.5) * canvas.width / cols);
